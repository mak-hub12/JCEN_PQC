...
#endif
#ifndef STACK_FALCON
#define STACK_FALCON    19200
#endif
#ifndef STACK_SPHINCS
#define STACK_SPHINCS   18432
#endif
#ifndef STACK_SENSOR
...
#endif
#if defined(BENCH_FALCON)
  #include "FALCON512_clean/api.h"
#endif
#if defined(BENCH_SPHINCS)
  #include "SPHINCSSHA2128SSIMPLE_clean/api.h"
#endif
...
#endif
#if defined(BENCH_FALCON)
static void FalconTask(void *arg);
#endif
#if defined(BENCH_SPHINCS)
static void SphincsTask(void *arg);
#endif
...
#elif defined(BENCH_FALCON)
    ok = xTaskCreate(FalconTask, "falcon", STACK_FALCON, NULL, PRIO_CRYPTO, NULL);
    uart_puts("[rtos] xTaskCreate Falcon -> "); uart_puts(ok==pdPASS ? "PASS\r\n" : "FAIL\r\n");
#elif defined(BENCH_SPHINCS)
    ok = xTaskCreate(SphincsTask, "sphincs", STACK_SPHINCS, NULL, PRIO_CRYPTO, NULL);
    uart_puts("[rtos] xTaskCreate SPHINCS+ -> "); uart_puts(ok==pdPASS ? "PASS\r\n" : "FAIL\r\n");
#else
    ok = xTaskCreate(P256Task, "p256", STACK_P256, NULL, PRIO_CRYPTO, NULL);
...
{
    (void)arg;
    const char *ALG = "falcon512";
    uart_puts("[task] FalconTask start\r\n");
    ...
    const size_t PKB = PQCLEAN_FALCON512_CLEAN_CRYPTO_PUBLICKEYBYTES;
    (void)PKB; // Used in csv_emit
    ...
    for (int i = 0; i < RUNS; i++) {
        ...
        ret = PQCLEAN_FALCON512_CLEAN_crypto_sign_signature(
                  sig, &siglen, (const uint8_t*)"falcon test", 11, sk);
        c1 = dwt_cycles(); mark_off();
        if (i >= WARMUP) csv_emit(ALG, "sign", 11, siglen, c1-c0, us_from_cycles(c1-c0), 0,
                                  uxTaskGetStackHighWaterMark(NULL), hb, xPortGetFreeHeapSize());
        if (ret) break;
        ...
        ret = PQCLEAN_FALCON512_CLEAN_crypto_sign_verify(
                  sig, siglen, (const uint8_t*)"falcon test", 11, pk);
        c1 = dwt_cycles(); mark_off();
        if (i >= WARMUP) csv_emit(ALG, (ret==0) ? "verify_ok" : "verify_fail",
                                  siglen, 0, c1-c0, us_from_cycles(c1-c0), 0,
                                  uxTaskGetStackHighWaterMark(NULL), hb, xPortGetFreeHeapSize());
    ...
    uart_puts("[falcon512] done\r\n");
    vTaskDelete(NULL);
}
#endif /* BENCH_FALCON */

/* =========================================================
 *                   SPHINCS+-SHA2-128s Task
 * ========================================================= */
#if defined(BENCH_SPHINCS)
static void SphincsTask(void *arg)
{
    (void)arg;
    const char *ALG = "sphincs_sha2_128s";
    uart_puts("[task] SphincsTask start\r\n");

    const size_t PKB = PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES;
    const size_t SKB = PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES;
    const size_t SSB = PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_BYTES;
    ...
    for (int i = 0; i < RUNS; i++) {
        ...
        hb = xPortGetFreeHeapSize();
        mark_on(); c0 = dwt_cycles();
        ret = PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_crypto_sign_keypair(pk, sk);
        c1 = dwt_cycles(); mark_off();
        if (i >= WARMUP) csv_emit(ALG, "keygen", 0, PKB, c1-c0, us_from_cycles(c1-c0), 0,
                                  uxTaskGetStackHighWaterMark(NULL), hb, xPortGetFreeHeapSize());
        if (ret) break;
        ...
        ret = PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_crypto_sign_signature(
                  sig, &siglen, (const uint8_t*)"sphincs test", 12, sk);
        c1 = dwt_cycles(); mark_off();
        if (i >= WARMUP) csv_emit(ALG, "sign", 12, siglen, c1-c0, us_from_cycles(c1-c0), 0,
                                  uxTaskGetStackHighWaterMark(NULL), hb, xPortGetFreeHeapSize());
        if (ret) break;

        hb = xPortGetFreeHeapSize();
        mark_on(); c0 = dwt_cycles();
        ret = PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_crypto_sign_verify(
                  sig, siglen, (const uint8_t*)"sphincs test", 12, pk);
        c1 = dwt_cycles(); mark_off();
        if (i >= WARMUP) csv_emit(ALG, (ret==0) ? "verify_ok" : "verify_fail",
                                  siglen, 0, c1-c0, us_from_cycles(c1-c0), 0,
                                  uxTaskGetStackHighWaterMark(NULL), hb, xPortGetFreeHeapSize());
    ...
    uart_puts("[sphincs_sha2_128s] done\r\n");
    vTaskDelete(NULL);
}
#endif /* BENCH_SPHINCS */
